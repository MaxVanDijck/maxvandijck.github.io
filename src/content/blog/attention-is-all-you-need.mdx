...
class Head(torch.nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    Q = self.linear_transform_Q(embedded_sentence) # batch, sequence, hidden_size
    K = self.linear_transform_K(embedded_sentence) # """
    V = self.linear_transform_V(embedded_sentence) # """
...
hidden_size = 64
heads = [Head(3, 64) for _ in range(3)]
linear = torch.nn.Linear(input_embedding_size * hidden_size, input_embedding_size * hidden_size)
mock_embedded_sentence = torch.randn(1, 3, input_embedding_size) # Batch, Sequence, Embedding

concat = torch.cat([head(mock_embedded_sentence) for head in heads], dim=2) # batch, sequence, hidden_size * num_heads
out = linear(concat)
```
